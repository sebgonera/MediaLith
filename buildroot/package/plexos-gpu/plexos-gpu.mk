...
# plexos-gpu
#
# The diagnostic the whole project exists for: does hardware transcoding actually
# work on this machine, and if not, exactly why.
#
# ARCHITECTURE.md section 6 states the failure it eliminates -- transcoding silently
# falls back to software, the CPU pins at 100%, playback stutters, and nothing in any
# log says why. A tool that reports "hardware acceleration unavailable" and stops has
# reproduced that problem rather than solved it, which is why every diagnostic it
# emits names a remedy, enforced by a test.
#
# It runs vainfo, so BR2_PACKAGE_LIBVA_UTILS is not optional decoration -- see
# Config.in.
#
...
PLEXOS_GPU_VERSION = 0.1.0
PLEXOS_GPU_SITE = $(BR2_EXTERNAL_PLEXOS_PATH)/..
PLEXOS_GPU_SITE_METHOD = local
...
PLEXOS_GPU_CARGO = $(firstword \
	$(shell command -v cargo 2>/dev/null) \
	$(wildcard $(HOME)/.cargo/bin/cargo))

define PLEXOS_GPU_BUILD_CMDS
	@if [ -z "$(PLEXOS_GPU_CARGO)" ]; then \
		echo "plexos-gpu: no cargo found on PATH or in \$$HOME/.cargo/bin"; \
		echo "  remedy: install the Rust toolchain (docs/DEVELOPMENT.md), or"; \
		echo "          set BR2_PACKAGE_PLEXOS_GPU=n to build an image without PID 1"; \
		exit 1; \
	fi
	cd $(@D) && $(PLEXOS_GPU_CARGO_ENV) \
		$(PLEXOS_GPU_CARGO) build \
			--release \
			--locked \
			--package plexos-gpu \
			--target $(PLEXOS_GPU_RUST_TARGET)
endef

PLEXOS_GPU_BINARY = \
	$(@D)/target/$(PLEXOS_GPU_RUST_TARGET)/release/plexos-gpu

define PLEXOS_GPU_INSTALL_TARGET_CMDS
	...
	@if readelf -l $(PLEXOS_GPU_BINARY) 2>/dev/null | grep -q INTERP; then \
		echo "plexos-gpu: built binary is dynamically linked"; \
		echo "  remedy: check RUSTFLAGS reached cargo; the initrd has no loader or libc"; \
		file -b $(PLEXOS_GPU_BINARY); \
		exit 1; \
	fi
	$(INSTALL) -D -m 0755 $(PLEXOS_GPU_BINARY) \
		$(TARGET_DIR)/usr/bin/plexos-gpu
endef
